...
The third wheel in the back was purchased in a local hardware store and the wheel itself was reprinted in TPU in order to run smoother on hard surfaces like concrete.
All the parts can be found in [this archive (MISSING)](./cad/motor_wheel_parts.zip).

![wheel_assembly1](./img/wheel_assembly1.png)
![wheel_assembly2](./img/wheel_assembly2.jpg)
![wheel_assembly3](./img/wheel_assembly3.jpg)
![wheel_assembly4](./img/wheel_assembly4.jpg)
![third_wheel_top](./img/third_wheel_top.png)
![third_wheel_bottom](./img/third_wheel_bottom.png)
![third_wheel1](./img/third_wheel1.jpg)
![third_wheel2](./img/third_wheel2.jpg)
...
The step files for the base can be found [HERE](./cad/gps_mount_base.step) and the one for the retainer can be found [HERE](./cad/gps_mount_retainer.step).
The config file for the GPS can be found [HERE](./config/uBlox_F9P_Rover_config.txt) and can be applied using the [u-Center](https://www.u-blox.com/en/product/u-center).
The config file for the radio module can be found [HERE](../base/config/GPS_RTCM_XBeeSX686_fwA00A.xpro) and should also be used on the [Base Station](base_station_gps).
The antenna is placed on a 200x200x2mm sheet of aluminium to filter out ground reflections.
...
So the whole setup consists of a the [four-way bay (MISSING)](./cad/bays.step), [the can clamps (MISSING)](./cad/can_clamp.step), [thumb screws (MISSING)](./cad/thumb_screws.step) and the [bottom cover (MISSING)](./cad/bottom_cover.step).
...
The parts to reprint this part can be found [in this archive (MISSING)](./cad/battery_port.zip).
...
![banana_mount1](./img/banana_plug_mount1.png)
...
Both used motor ports M3, M4 were configured like this.

![esc_config](./img/BLHeli32_ESC_config.png)

### Pixhawk

As this is the heart of the project a lot of different configurations were used. All of them are combined in the parameter file linked below.
Details are explained in the following sections.

#### UART

The Pixhawk has 6 UARTs, which are used for USB (UART0), ELRS radio module communication (UART1), host telemetry back to MissionPlanner (UART2), and GPS data (UART3).
The last two UARTs are not used.

![config_uart](./img/config_uart.png)

#### Servo Output

The servo output is one of the relevant parts of the rover.
The first four channels belong to the spray pod servos.
They are all linked to two certain RC input channels, which map to a a push button on the RC controller to manually fire all spray pods (at once). This is mainly used for testing or for manually marking spots once the rover moved there.
Special attention needs to be paid to the trim value, which represents the idle state of each servo.
As the servos in the pods are mirrored, the values change from pod to pod.
Ouput channel 9 and 10 are used for the motor controller.
As the motor can also go backwards, 1500 is the idle/standstill value.

![config_servo](./img/config_servo.png)

#### RC Input

As the remote control also sets other options on the pixhawk, a few more RCINx channels are mapped to special functions. The mode the rover is supposed to go in (manual/auto) in order to start or pause executing a mission.

![config_rcin5](./img/config_rcin5.png)

There is also the arming setting, which is set with another toggle switch on the remote.

![config_rcin6](./img/config_rcin6.png)

#### Power Measurement

To have an overview over the power consumption and battery voltage a power meter is used. The exact battery capacity varied over the development and is not used for any sort of alarm. The configuration looks like this:

![config_power](./img/config_power.png)

## Links

[ArduPilot Config File](./config/PixhawkArduPilot.param)
